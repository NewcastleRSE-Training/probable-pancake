...
#### Try out a dry run:
```bash
`r config$remote$prompt` cd /nobackup/proj/training/`r config$remote$user`/
`r config$remote$prompt` mkdir TestDir
`r config$remote$prompt` touch TestDir/testfile1
`r config$remote$prompt` touch TestDir/testfile2
`r config$remote$prompt` rsync -av TestDir /rdw/04/rse-training/`r config$remote$user` --dry-run
```
```output
...
#### Run ‘for real’:
```bash
`r config$remote$prompt` rsync -av TestDir /rdw/04/rse-training/`r config$remote$user`
```
```output
sending incremental file list
created directory /rdw/04/rse-training/`r config$remote$user`
rsync: chgrp "/rdw/04/rse-training/`r config$remote$user`/TestDir" failed: Invalid argument (22)
TestDir/
TestDir/testfile1
TestDir/testfile2
rsync: chgrp "/rdw/04/rse-training/`r config$remote$user`/TestDir/.testfile1.ofeRqX" failed: Invalid argument (22)
rsync: chgrp "/rdw/04/rse-training/`r config$remote$user`/TestDir/.testfile2.fS1m6j" failed: Invalid argument (22)
...
```bash
`r config$remote$prompt` ls -l TestDir/
```
```output
total 0
-rw------- 1 `r config$remote$user` comet_training 0 Mar 11 20:06 testfile1
-rw------- 1 `r config$remote$user` comet_training 0 Mar 11 20:06 testfile2

```
```bash
`r config$remote$prompt` ls -l /rdw/04/rse-training/`r config$remote$user`/TestDir/
```
```output
total 33
-rwxrwx--- 1 `r config$remote$user` domainusers 0 Mar 11 20:10 testfile1
-rwxrwx--- 1 `r config$remote$user` domainusers 0 Mar 11 20:10 testfile2
```
...
```bash
`r config$remote$prompt` rsync -rltv TestDir /rdw/04/rse-training/`r config$remote$user`/ 
```
```output
sending incremental file list
./
...
:::solution
```bash
`r config$remote$prompt` ls -R /rdw/04/rse-training/`r config$remote$user`/
```
```output
/rdw/04/rse-training/`r config$remote$user`/:
TestDir  testfile1  testfile2

/rdw/04/rse-training/`r config$remote$user`/TestDir:
testfile1  testfile2
...
:::solution
```bash
[`r config$remote$user`@login01 ~]$ rsync -rltv --inplace --size-only DataDir /rdw/04/rse-training/`r config$remote$user`/
```
