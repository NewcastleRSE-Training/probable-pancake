...
`r config$sched$comment` --partition=default_free
`r config$sched$comment` --job-name=`r config$sched$name`
`r config$sched$comment` --nodes=1
`r config$sched$comment` --tasks=1
`r config$sched$comment` --cpus-per-task=1
...
`r config$remote$bash_shebang`
`r config$sched$comment` --partition=default_free
`r config$sched$comment` --job-name=parallel-job
`r config$sched$comment` --nodes=1
`r config$sched$comment` --tasks=1
`r config$sched$comment` --cpus-per-task=4
...
`r config$sched$comment` --job-name=parallel-job
`r config$sched$comment` --tasks=1
`r config$sched$comment` --cpus-per-task=8
