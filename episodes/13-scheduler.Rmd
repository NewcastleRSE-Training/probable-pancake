...
```bash
`r config$remote$prompt` `r config$sched$submit$name` `r config$sched$submit$options``r config$sched$partition` example-job.sh
```
...
```bash
`r config$remote$bash_shebang`
`r config$sched$comment` --account=comet_training
...
```bash
`r config$remote$prompt` `r config$sched$interactive` `r config$sched$flag$queue``r config$sched$partition` hostname
```
...
```bash
`r config$remote$prompt` `r config$sched$interactive` `r config$sched$flag$queue``r config$sched$partition` -n 2 echo "This job will use 2 CPUs."
```
...
```bash
`r config$remote$prompt` `r config$sched$interactive` `r config$sched$flag$queue`--account=comet_training `r config$sched$partition` --pty bash
```
